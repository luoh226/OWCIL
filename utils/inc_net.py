...
class BaseNet(nn.Module):
    def __init__(self, args, pretrained):
        ...
        self.separate_head = args["separate_head"]
        self.separate_type = args["separate_type"]
        if self.separate_head:
            self.fc_edl = None      # 将ECE分开到另一个头上训，原头用CE训
...
class IncrementalNet(BaseNet):
    ...
    def update_fc(self, nb_classes, edl_nb_classes=None):
        ...
        if self.separate_head:
            fc_edl = self.generate_fc(self.feature_dim, edl_nb_classes if edl_nb_classes else nb_classes)
            if self.fc_edl is not None:
                nb_output = self.fc_edl.out_features
                weight = copy.deepcopy(self.fc_edl.weight.data)
                bias = copy.deepcopy(self.fc_edl.bias.data)
                fc_edl.weight.data[:nb_output] = weight
                fc_edl.bias.data[:nb_output] = bias
    ...
    def forward(self, x):
        ...
        if self.separate_head:
            if self.separate_type == 1:
                logits = self.fc(feature.detach())["logits"]
                logits_edl = self.fc_edl(feature)["logits"]
            elif self.separate_type == 2:
                logits = self.fc(feature)["logits"]
                logits_edl = self.fc_edl(feature.detach())["logits"]
            elif self.separate_type == 3:
                logits = self.fc(feature)["logits"]
                logits_edl = self.fc_edl(feature)["logits"]
            else:
                raise "ERROR: separate_type: {} is not defined!".format(self.separate_type)
            out = {"feature": feature, "logits": logits, "logits_edl": logits_edl}
        else:
...
class IL2ANet(IncrementalNet):
    ...
    def update_fc(self, num_old, num_total, num_aux):
        ...
        if self.separate_head:
            fc_edl = self.generate_fc(self.feature_dim, num_total)
            if self.fc_edl is not None:
                nb_output = self.fc_edl.out_features
                weight = copy.deepcopy(self.fc_edl.weight.data)
                bias = copy.deepcopy(self.fc_edl.bias.data)
...
class CosineIncrementalNet(BaseNet):
    ...
    def update_fc(self, nb_classes, task_num):
        ...
        if self.separate_head:
            fc_edl = self.generate_fc(self.feature_dim, nb_classes, edl=True)
            if self.fc_edl is not None:
                nb_output = self.fc_edl.out_features
                weight = copy.deepcopy(self.fc_edl.weight.data)
                bias = copy.deepcopy(self.fc_edl.bias.data)
    ...
    def generate_fc(self, in_dim, out_dim, edl=False):
        if edl:
            fc = SimpleLinear(in_dim, out_dim)
        else:
            if self.fc is None:
...
class EFCILNet(nn.Module):
    def __init__(self, args, pretrained):
        super(EFCILNet, self).__init__()
        self.convnet_type = args["convnet_type"]
        self.convnets = nn.ModuleList()
        self.pretrained = pretrained
        self.out_dim = None     # 最新的backbone输出的logit维度 (512)
        self.fcs = nn.ModuleList()   # 与backbone一样保存每个task的classifiers
        self.separate_head = args["separate_head"]
        self.separate_type = args["separate_type"]
        if self.separate_head:
            self.fcs_cnn = nn.ModuleList()      # 将ECE分开到另一个头上训，原头用CE训
        self.args = args
    ...
    def forward(self, x):
        assert len(self.convnets) == len(self.fcs), 'Error: the number of backbone and fc layer is different!'
        if self.separate_head:
            assert len(self.fcs_cnn) == len(self.fcs), 'Error: the number of fc_cnn and fc layer is different!'

        features = [convnet(x)["features"] for convnet in self.convnets]   # [(128, 512), (128, 512), ...] list(tensor)

        if not self.separate_head:
            logits = [fc(features[i])["logits"] for i, fc in enumerate(self.fcs)]  # [(128, 10), (128, 10), ...] list(tensor)
            out = {"features": features, "logits": logits}
        else:
            if self.separate_type == 1:
                logits = [fc(features[i])["logits"] for i, fc in enumerate(self.fcs)]
                logits_cnn = [fc(features[i].detach())["logits"] for i, fc in enumerate(self.fcs_cnn)]
            elif self.separate_type == 2:
                logits = [fc(features[i].detach())["logits"] for i, fc in enumerate(self.fcs)]
                logits_cnn = [fc(features[i])["logits"] for i, fc in enumerate(self.fcs_cnn)]
            elif self.separate_type == 3:
                logits = [fc(features[i])["logits"] for i, fc in enumerate(self.fcs)]
                logits_cnn = [fc(features[i])["logits"] for i, fc in enumerate(self.fcs_cnn)]
            else:
                raise "ERROR: separate_type: {} is not defined!".format(self.separate_type)
            out = {"features": features, "logits": logits, "logits_cnn": logits_cnn}
    ...
    def update_fc(self, nb_classes):
        ...
        if self.separate_head:
            fc_cnn = self.generate_fc(self.feature_dim, nb_classes)
            self.fcs_cnn.append(fc_cnn)
