...
class IL2A(BaseLearner):
    ...
    def incremental_train(self, data_manager, fold='final'):
        ...
        self._network.update_fc(self._known_classes, self._total_classes,
                                int((self._current_classes - 1) * self._current_classes / 2))
    ...
    def _train_function(self, train_loader, test_loader, optimizer, scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                outputs, loss_clf, loss_fkd, loss_proto = self._compute_il2a_loss(inputs, targets.long())
                loss = loss_clf + loss_fkd + loss_proto

                if self._separate_head:
                    ori_batch = inputs.shape[0]
                    loss = loss * self.args["b2"]
                    logits_edl = outputs["logits_edl"]
                    loss_edl = self.edl_loss(logits_edl[:ori_batch, self._known_classes:], targets[:ori_batch] - self._known_classes,
    ...
    def _compute_il2a_loss(self, inputs, targets):
        ...
        proto_logits = self._semantic_aug(proto_logits, proto_targets.long(), self.args["ratio"])
    ...
    def _semantic_aug(self, proto_logits, proto_targets, ratio):
        ...
        N_target_weight = torch.gather(N_weight, 1, proto_targets[:, None, None].expand(N, C, D).long())  # NCD
    ...
    def _eval_cnn(self, loader, val=False):
        ...
        for _, (_, inputs, targets) in enumerate(loader):
            ...
            with torch.no_grad():
                ...
                # else:
                outputs = self._network(inputs)
            prob = torch.softmax(outputs["logits"][:, :self._total_classes], dim=1)
            ...
            if self._ood_method != "None":
                if "MSP" == self._ood_method:
                    ...
                    ood_prob = -torch.logsumexp(outputs["logits"][:, :self._total_classes], dim=1)
